...
//		stepDetector.load();
	...
	@Override
	public void onStepDetected(long now_ms, double compDir, double stepLength) {
		...
		float dx=(float) (Math.sin(angle)*stepLength)*locationService.getGridSpacingX();
		float dy=(float) (Math.cos(angle)*stepLength)*locationService.getGridSpacingY();
