...
/**
 * This class is fed with data from the Accelerometer and Compass sensors. If a step is detected on the acc data
 * it calls the trigger function on its interface StepTrigger, with the given direction.
...
public class StepDetection implements CompassListener {
	...
	protected synchronized void updateData() {
		...
		if (detector.checkForStep()) {
			...
			double stepLength = detector.getStepLength();
			st.onStepDetected(now_ms, lastComp, stepLength);
	...
	public void onCompassChanged(float azimuth, float angle, String direction) {
		...
		Logger.d("Current Heading: " + azimuth);
	}
}
